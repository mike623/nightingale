...
- A phone on the same network can now control playback. Turn on Remote control in Settings → Playback and scan the QR code shown there: the phone opens a touch page with transport, guide-vocals level in 10% steps, lyrics, background, microphone, and skip intro/outro. The phone sends the same commands the keyboard shortcuts do, so both surfaces behave identically. Every phone on the page may act — a karaoke room is a group around one screen — and audio never leaves the host screen. Remote control is off by default, and on the desktop app nothing listens on the network until it is switched on and the port is released again when it is switched off — anyone who can reach the address can control playback and change the queue, so it should stay off on untrusted networks.
...
- A phone can now start the singing when nothing is playing: the Playing tab offers Play something, and the host draws a song with the same weighted pick it uses to choose the next one, favouring what the room has not sung lately. The host answers this from its menu, settings and import screens, so nobody has to be at the keyboard to get a party going; a host with no analyzed song says so on its own screen, which is the screen the room is watching. On the desktop app this means the listener now starts when the app does, rather than waiting for the QR code to be opened — still only while Remote control is on, and the switch releases the port as before. The desktop also refuses to bind the port at all while that setting is off.
- The phone remote page has a Lyrics tab showing the words of the song that is playing, read from the host's own transcript. When the host turns on Shift lyrics from a phone in Settings → Playback, the tab also carries the lyrics editor's ±0.1s, ±0.5s and ±1s buttons: they move the words on the karaoke screen earlier or later to match what the room hears, up to ten seconds either way. The shift lasts for the song being played — the audio, the scoring and the stored lyrics keep their own timing, and the next song starts unshifted. The buttons are off by default, because the remote page is unauthenticated and a shift changes what everyone in the room reads.
- The phone remote page now spans the whole viewport instead of stopping at a centred column, so a tablet in landscape shows the library across the full screen rather than between two gutters.
- The playback queue now survives a restart. It is saved as `playback_queue.json` in the data folder and read back when Nightingale starts, keeping each entry's position, tempo, and key shift, so closing the app mid-party no longer empties the room's line-up. A queued song that has since left the library is dropped, because nothing could play it.
...
- The phone remote's Library tab now scrolls the whole library in one list instead of ending each page with a Show more button. The host says how many songs match and the list reserves the room for all of them, keeping only the rows near the screen in the page, so a library of any size stays as quick to scroll near the end as at the start. Songs are fetched a page at a time for wherever the list has been scrolled to rather than one page after another from the top, and a row whose song has not arrived holds its place rather than shifting the list when it does. The search box now stays in reach above the list rather than scrolling away with it.
...
- A phone on the remote page can paste a YouTube link to add a song to the library. The host downloads it and analyses it through the normal pipeline, and the phone watches it happen: an Add tab lists every import the host is working through, with a progress bar while each one downloads. The song joins the library like any other, to be queued from the Library tab when it is wanted. Only single videos are accepted — playlists still have to be imported from the app — and the phone is told whether a video could not be downloaded but never the host's own error text, which quotes paths on that machine. This is off by default and appears in Settings → Playback only while remote control is on. Anyone who can reach the address can use it: it makes the host run a download and write to your library folder, so leave it off unless you trust the room.
- The import page is now one screen: a two-line box for links above a table of the import queue. Every video is a row carrying its title, artist, length, and where it has got to — about to import, queued, downloading, imported, already on disk, or failed — instead of the page swapping between a full-height box, a pick list, and a progress list. A video's length is shown for the first time, and title and artist stay editable until the row is queued. Finished rows stay until they are cleared, so a run can still be read after it ends, and failed rows can be queued again from the same screen.
- Imports now go through a queue that survives leaving the page and closing the app. Submitting a second import while one is running queues it behind the first instead of starting a competing run: two runs at once each saved a record of the folder taken before the other's downloads existed, and the later save dropped the earlier one's entries, so those videos were downloaded again on the next import. Downloads that were in flight when the app closed go back in the queue, and a video already on disk is skipped without downloading. Within one import, videos still download three at a time.
